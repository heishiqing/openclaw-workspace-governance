...
用这份 skill，你可以把一个“能用但容易漂”的工作区，收成一个更稳定的系统：
- 有明确的治理边界
- 有清楚的权威事实层（source-of-truth layers）
- 能按问题类型决定检索顺序
- 能区分 live docs 和 historical docs
- 有 working set（工作集）和 freshness checks（保鲜检查）
- 能在主线落地后进入 maintenance-mode closure（维护收口）
...
Use this skill to turn a workspace from "usable but drift-prone" into one with:
- explicit governance boundaries
- explicit source-of-truth layers
- query-class-aware retrieval routing
- live vs historical doc separation
- freshness working sets and recurring checks
- maintenance-mode closure instead of endless process sprawl
